...
// NOTE: these were previously mutable top-level `List<String>` variables.
// Since they're public, any code importing this file could mutate them
// (`allowedExtensions.add(...)`, `.clear()`, etc.) and silently affect every
// other picker instance in the app. Making them `const` prevents that and
// lets the compiler share a single constant list instance instead of
// allocating a new growable list at startup.
const List<String> allowedExtensions = [
  ...
  '.wbs', // was 'wbs' (missing leading dot) — inconsistent with every other entry and wouldn't match a real extension check.
...
const List<String> allowedExtensionsVideo = [
  // Video files
  '.mp4', '.avi', '.mov', '.wmv', '.flv', '.mkv',
];

const List<String> allowedExtensionsAudio = [
  // Audio files
  '.mp3', '.wav', '.aac', '.ogg', '.flac', '.amr', '.m4a', '.wma',
];

Future<List<DirectoryType>> getPublicDirectories() {
...
// Previously, `getIconForFile` built these two ~80-entry maps from scratch
// on *every call* — and it's called once per file tile every time the file
// list/grid builds. Hoisting them to module-level `const` maps means the
// lookup tables are built exactly once (at compile time, even), and each
// call to `getIconForFile` becomes a couple of O(1) map lookups.
const Map<String, IconData> _fileIconByExtension = {
...
const Map<String, Color> _fileColorByExtension = {
  '.doc': Colors.blue,
  '.docx': Colors.blue,
  '.rtf': Colors.blueGrey,
  '.txt': Colors.grey,
...
Widget getIconForFile(String extension) {
  final key = extension.toLowerCase();
  final iconData = _fileIconByExtension[key] ?? Icons.insert_drive_file;
  final color = _fileColorByExtension[key] ?? Colors.grey;
